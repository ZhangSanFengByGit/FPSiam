...
def get_subwindow_tracking(im, pos, model_sz, original_sz, avg_chans, out_mode='torch', new=False):
    ...
    assert np.argwhere(np.isnan(im)).size==0, '{}'.format(np.argwhere(np.isnan(im)).size)
    assert np.argwhere(np.isinf(im)).size==0, '{}'.format(np.argwhere(np.isinf(im)).size)
    ...
    assert np.argwhere(np.isnan(im_patch)).size==0, '{}'.format(np.argwhere(np.isnan(im_patch)).size)
    assert np.argwhere(np.isinf(im_patch)).size==0, '{}'.format(np.argwhere(np.isinf(im_patch)).size)

    out = im_to_torch(im_patch) if out_mode in 'torch' else im_patch
